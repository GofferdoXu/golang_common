...
func InitPGDBPool(path string) error {
	//普通的db方式
	DbConfMap := &PGSqlMapConf{}
	err := ParseConfig(path, DbConfMap)
	if err != nil {
		return err
	}
	if len(DbConfMap.List) == 0 {
		fmt.Printf("[INFO] %s%s\n", time.Now().Format(TimeFormat), " empty postgres config.")
	}

	DBMapPool = map[string]*sql.DB{}
	GORMMapPool = map[string]*gorm.DB{}
	for confName, DbConf := range DbConfMap.List {
		println("host=" + DbConf.Path + " user=" + DbConf.Username + " password=" +
			DbConf.Password + " dbname=" + DbConf.Dbname + " port=" + DbConf.Port+ " " + DbConf.Config)
		dbpool, err := sql.Open("postgres", "host=" + DbConf.Path + " user=" + DbConf.Username + " password=" +
			DbConf.Password + " dbname=" + DbConf.Dbname + " port=" + DbConf.Port+ " " + DbConf.Config)
		if err != nil {
			return err
		}
	...
	//手动配置连接
	if dbpool, err := GetPGDBPool("default"); err == nil {
		DBDefaultPool = dbpool
	}
	if dbpool, err := GetPGGormPool("default"); err == nil {
...
func PGDBPoolLogQuery(trace *TraceContext, sqlDb *sql.DB, query string, args ...interface{}) (*sql.Rows, error) {
	...
	if err != nil {
		Log.TagError(trace, "_com_postgres_success", map[string]interface{}{
			"sql":       query,
			"bind":      args,
			"proc_time": fmt.Sprintf("%f", endExecTime.Sub(startExecTime).Seconds()),
		})
	} else {
		Log.TagInfo(trace, "_com_postgres_success", map[string]interface{}{
			"sql":       query,
			"bind":      args,
			"proc_time": fmt.Sprintf("%f", endExecTime.Sub(startExecTime).Seconds()),
		})
...
func (mgl *PGSqlGormLogger) Info(ctx context.Context, message string, values ...interface{}) {
	...
	Log.TagInfo(trace, "_com_postgres_Info", params)
}
func (mgl *PGSqlGormLogger) Warn(ctx context.Context, message string, values ...interface{}) {
	...
	params["message"] = message
	params["values"] = fmt.Sprint(values)
	Log.TagInfo(trace, "_com_postgres_Warn", params)
}
func (mgl *PGSqlGormLogger) Error(ctx context.Context, message string, values ...interface{}) {
	...
	params["message"] = message
	params["values"] = fmt.Sprint(values)
	Log.TagInfo(trace, "_com_postgres_Error", params)
}
func (mgl *PGSqlGormLogger) Trace(ctx context.Context, begin time.Time, fc func() (sql string, rowsAffected int64), err error) {
	...
	case err != nil && mgl.LogLevel >= logger.Error && (!errors.Is(err, logger.ErrRecordNotFound)):
		msg["err"] = err
		if rows == -1 {
			Log.TagInfo(trace, "_com_postgres_failure", msg)
		} else {
			msg["rows"] = rows
			Log.TagInfo(trace, "_com_postgres_failure", msg)
		}
	case elapsed > mgl.SlowThreshold && mgl.SlowThreshold != 0 && mgl.LogLevel >= logger.Warn:
		...
		msg["slowLog"] = slowLog
		if rows == -1 {
			Log.TagInfo(trace, "_com_postgres_success", msg)
		} else {
			msg["rows"] = rows
			Log.TagInfo(trace, "_com_postgres_success", msg)
		}
