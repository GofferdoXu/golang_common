...
func Init(configPath string) error {
	return InitModule(configPath, []string{"base", "mysql", "redis", "postgres"})
}
...
func InitModule(configPath string, modules []string) error {
	...
	// 加载postgres配置并初始化实例
	if InArrayString("postgres", modules) {
		if err := InitPGDBPool(GetConfPath("postgres_map")); err != nil {
			fmt.Printf("[ERROR] %s%s\n", time.Now().Format(TimeFormat), " InitPGDBPool:"+err.Error())
		}
	}
